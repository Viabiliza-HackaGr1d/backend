...
class AssuranceController {
  ...
  async update(req, res) {
    ...
    if (assurance.status === 4) {
      return res
        .status(401)
        .json({ error: 'This assurance has already been cancelled.' });
    }
  ...
  async delete(req, res) {
    ...
    if (req.userId !== customer_id || req.userId !== broker_id) {
      return res
        .status(401)
        .json({ error: "You're not allowed to delete this assurance." });
    }

    if (assurance.status !== 3) {
      return res
        .status(400)
        .json({ error: 'This assurance is not pending to be cancelled.' });
    }

    if (assurance.cancelled_by === req.userId) {
      return res
        .status(401)
        .json({ error: "An user can't cancel its own pending cancellation." });
    }
